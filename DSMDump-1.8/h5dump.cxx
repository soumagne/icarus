...
#endif
/*-------------------------------------------------------------------------*/
int H5dump_xml(std::ostringstream &stream, const char *filename)
{
...
/*-------------------------------------------------------------------------*/
int H5dump_dsm_light(void *dsmBuffer)
{
    const char *argv[5]={"./h5dump", "-H", "-f", "dsm", "DSM.h5"};
    output_stream.str("");
    output_stream.clear();
...
/*-------------------------------------------------------------------------*/
int H5dump_dsm_xml(std::ostringstream &stream, void *dsmBuffer)
{
    // h5dump -x -X : -H -f dsm DSM.h5
    const char *argv[8] = {"./h5dump", "-x", "-X", ":", "-H", "-f", "dsm", "DSM.h5"};
    output_stream.str("");
    output_stream.clear();
    std::streambuf* old_buf = std::cout.rdbuf(output_stream.rdbuf());
